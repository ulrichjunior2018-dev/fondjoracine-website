...
function buildAdminNotificationHtml(payload: OrderPlacedNotification): string {
...
/**
 * Emails the back-office when a manual-payment order is placed. Stripe orders
 * are confirmed via webhook, so they are intentionally skipped here.
 */
export const adminEmailChannel: NotificationChannel = {
  ...
  async notifyOrderPlaced(event) {
    ...
    if (event.paymentMethod.toLowerCase() === "stripe") {
      return;
    }
